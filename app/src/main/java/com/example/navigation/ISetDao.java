...
public interface ISetDao {
    ...
    @Transaction
    @Query("SELECT set_table.id,complete,displayNumber,MAX(weight) as weight, MAX(reps) as reps FROM set_table,user_routine_exercise_table WHERE set_table.userRoutineExerciseRoutineId = user_routine_exercise_table.id AND user_routine_exercise_table.exerciseTypeId=:exerciseId")
